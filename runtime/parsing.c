...
value parse_engine(struct parser_tables *, struct parser_env *, value, value);

value parse_engine(struct parser_tables *tables, struct parser_env *env,
		   value cmd, value arg)
{
	static int state;
	static mlsize_t sp;
	...
	shift:
		...
		if (sp < (unsigned) VAL_TO_LONG(env->stacksize)) {
			goto push;
		}
