...
type QualifiedIdent = { id: string list,
			qual: string }
...
(* Id is used distinguish between name spaces
   for compiled variable, structure and functor values *)

datatype Id = ModId of string | ValId of string | FunId of string

(* (un)mangle ValId's, ModId's and FunId's to disjoint subsets of string *)
...
in
fun printQualId {qual="", id=id} = print_id id
  | printQualId {qual=qual, id=id} = (msgString qual;  msgString "." ; print_id id)
end
