...
void remove_conflicts(void)
{
	...
	for (p = parser[i]; p; p = p->next)
	{
	    ...
	    /* Modified to use precedence to resolve r/r conflicts */
	    /* by sestoft@dina.kvl.dk 1998-09-24 */
	    else if (pref->prec > 0 && p->prec > 0)
		{
		    if (pref->prec < p->prec)
		    {
			pref->suppressed = 2;
		    ...
		    else /* Same precedence - unresolved r/r conflict */
		      {
			RRcount++;
			p->suppressed = 1;
		      }
		}
	    else /* No precedence - unresolved r/r conflict */
	    {
		RRcount++;
		p->suppressed = 1;
	    }
...
int sole_reduction(int stateno)
{
	...
	if (p->action_code == SHIFT)
	  /* Modified by sestoft 1998-03-18 to fix associativity bug.
	     Return 0 even if the SHIFT action has been suppressed: */
	  return (0);
