...
{
    fprintf(stderr, "%s: error   - line %d of \"%s\", the entry point %s is a \
token\n", myname, lineno, input_file_name, s);
    done(1);
}
...
{
    fprintf(stderr, "%s: error   - line %d of \"%s\", more than 256 entry points\n",
            myname, lineno, input_file_name);
    done(1);
}
...
{
    fprintf(stderr, "%s: error   - line %d of \"%s\", $%d is unbound\n",
...
{
    fprintf(stderr, "%s: error   - line %d of \"%s\", $%d refers to terminal `%s', which has no argument\n",
...
void missing_type(char *s)
{
    fprintf(stderr, "%s: error   - the symbol '%s' has no %%type declaration\n", myname, s);
    done(1);
}
