...
fun zipEq (xs, ys) =
    let
      fun h (x::xr) (y::yr) res = h xr yr ((x, y) :: res)
	| h []      []      res = List.rev res
...
fun mapEq f (xs, ys) =
    let
      fun h (x :: xr) (y :: yr) res = h xr yr (f(x, y) :: res)
	| h [] [] r = List.rev r
	| h _ _ _ = raise UnequalLengths
    in
      h xs ys []
...
fun allEq p (xs, ys) =
    let
      fun h (x :: xr) (y :: yr) = p(x, y) andalso h xr yr
	| h []        []        = true
	| h _         _         = raise UnequalLengths
    in
      h xs ys
...
fun foldlEq f e (xs, ys) =
    let
      fun h e (x :: xr) (y :: yr) = h (f(x, y, e)) xr yr
	| h e []        []        = e
	| h _ _         _         = raise UnequalLengths
    in
      h e xs ys
