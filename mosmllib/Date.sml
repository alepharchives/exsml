(* Date -- 1995-07-03, 1998-04-07, 1999-08-04 *)
...
datatype date = DATE of {
    ...
    offset : int option			(* signed seconds t East of UTC: this
...
local
    ...
    prim_val localoffset_  : unit -> real = 1 "sml_localoffset";

    (* The offset to add to local time to get UTC: positive West of UTC *)

    val localoffset = localoffset_ ()
	...
	if month = Feb andalso leapyear year then 29
	else Vector.sub(monthlengths, frommonth month)
...
in
	...
	Option.map (fn secs => Time.fromSeconds ((86400 - secs) mod 86400))
	           offset
end
